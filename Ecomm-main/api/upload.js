...
const baseUrl = "https://viaanmart.herokuapp.com/image";
...
router.post("/addfiles", upload.single("file"), async (req, res) => {
  try {
    ...
    //     let hr = await AdminSchemas.findById(adminkey);
    //    if(!hr){
    ...
    if (req.file) {
      ...
      let filel = await firebase.bucket
        .file(`files/` + file.originalname)
        .getSignedUrl({
          action: "read",
          expires: "03-09-2491",
        });
